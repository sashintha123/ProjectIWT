<?php
    ...
    if(isset($_POST["login-btn"])){
        ...
        if(inputsEmptyLogin($email, $pass)){
            header("location:leclogin.php?err=empty_inputs");
        }
        else if(emailInvalid($email)){
            header("location:leclogin.php?err=invalid_email");
        }
        else if(passwordInvalid($pass)){
            header("location:leclogin.php?err=invalid_password");
        }
        else{
    ...
    function loginUser($conn, $email, $pass, $remember){
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){
            header("location:leclogin.php?err=failedstmt");
        }
        else{
            ...
            if($row = mysqli_fetch_assoc($data)){
                ...
                if($isPassOk){
                    ...
                    header("location:leclogin.php?err=loginfailedpass");
                    exit();
                }
            }
            else{
                header("location:leclogin.php?err=loginfailedemail");
                exit();
            }
        }
        // Close the statement
