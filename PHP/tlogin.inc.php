<?php
    ...
    if(isset($_POST["login-btn"])){
        ...
        if(inputsEmptyLogin($email, $pass)){
            header("location:trailogin.php?err=empty_inputs");
        }
        else if(emailInvalid($email)){
            header("location:trailogin.php?err=invalid_email");
        }
        else if(passwordInvalid($pass)){
            header("location:trailogin.php?err=invalid_password");
        }
        else{
    ...
    function loginUser($conn, $email, $pass, $remember){
        // Query
        $sql = "SELECT * FROM trainee WHERE email = ?;";
        // Initialize the prepared statement
        $stmt = mysqli_stmt_init($conn);
        // Bind the statement with the query and check errors
        if(!mysqli_stmt_prepare($stmt, $sql)){
            header("location:trailogin.php?err=failedstmt");
        }
        else{
            ...
            if($row = mysqli_fetch_assoc($data)){
                ...
                if($isPassOk){
                    ...
                    header("location:trailogin.php?err=loginfailedpass");
                    exit();
                }
            }
            else{
                header("location:trailogin.php?err=loginfailedemail");
                exit();
            }
        }
        // Close the statement
