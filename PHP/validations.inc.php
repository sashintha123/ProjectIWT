<?php
    ...
    function inputsEmptyRegister($fname, $lname, $email, $mobile,$subject,$service, $pass, $re_pass){
        $value;
        if(empty($fname) || empty($lname) || empty($email) || empty($mobile) ||empty($subject)||empty($service)|| empty($pass) || empty($re_pass)){
            $value = true;
        }else{
            $value = false;
        }
    ...
    function emailOrMobileAvailable($conn, $email, $mobile){
        $value;
        // Query
        $sql = "SELECT * FROM lecturers WHERE email = ? OR mobile = ?;";
